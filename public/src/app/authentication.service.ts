...
export class AuthenticationService {
  ...
  getId() {
    if (this.isAuthenticated())
      return this._jwtService.decodeToken(this.getToken())._id;
    return 'invalid';
  }
  getToken() {
    if (this.isAuthenticated())
  ...
  isAdmin() {
    if (this.isAuthenticated())
      return this._jwtService.decodeToken(this.getToken()).isAdmin;
    return this.#isAdmin;
  }
  getName() {
    if (this.isAuthenticated())
      return this._jwtService.decodeToken(this.getToken()).name;
    return this.#name;
  }
  isLoggedIn() {
